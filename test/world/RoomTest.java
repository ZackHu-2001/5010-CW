...
public class RoomTest {
  ...
  public void testInvalidInputForConstructor() {
    try {
      new Room("FOO", new int[] {1, 2, 3});
    } catch (IllegalArgumentException e) {
      System.out.println("Successfully caught exception.");
    }

    try {
      new Room("FOO", new int[] {4, 2, 3, 3});
    } catch (IllegalArgumentException e) {
      System.out.println("Successfully caught exception.");
    }

    try {
      new Room("FOO", new int[] {4, 4, 5, 3});
    } catch (IllegalArgumentException e) {
      System.out.println("Successfully caught exception.");
    }

    try {
      new Room("FOO", new int[] {-1, 2, 3, 3});
    } catch (IllegalArgumentException e) {
      System.out.println("Successfully caught exception.");
    }
  ...
  public void addNeighbor() {
    ...
    for (int i = 0; i < 4; i++) {
      ...
      int c = b + random.nextInt(10);
      int d = c + random.nextInt(10);
      room.addNeighbor(new Room("Pen", new int[] {a, b, c, d}));
  ...
  public void getLocation() {
    ...
    for (int i=0; i<20; i++) {
      ...
      int c = b + random.nextInt(10);
      int d = c + random.nextInt(10);
      Room room = new Room("FOO" + String.valueOf(random.nextInt()), new int[]{a, b, c, d});
  ...
  public void testEquals() {
    ...
    Room a = new Room("Kitchen", new int[] {1, 2, 3, 4});
    Room b = new Room("Washroom", new int[] {2, 3, 4, 5});
    Room c = new Room("Living Room", new int[] {2, 3, 4, 5});
